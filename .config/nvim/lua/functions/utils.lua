local function open_std_float(lines, height)
    if not height then
        height = 15
    end
    ...
    local win = vim.api.nvim_open_win(
        buf
        , true
        , {relative = 'win', row=3, col=10, width=100, height=height}
    )
...
local function print_table(in_table)
    ...
    local buf, table_win = open_std_float(lines)
    vim.keymap.set(
        'n'
        , '-'
        , function()
